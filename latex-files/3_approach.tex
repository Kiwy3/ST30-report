...
\begin{algorithm}[h]
    ...
    \For{$i \gets 1$ \KwTo $n_{\text{opt}}$}{
        ...
        $\mathcal{D} \gets \mathcal{D} \cup \{(\lambda', f(\lambda'))\}$ \tcp{Evaluate new point}
    }
...
\begin{algorithm}[h]
    ...
    \While{$n < n_{\text{max}}$}{
        ...
        \For{$h \gets 0$ \KwTo $\text{depth}(\mathcal{T}_n)$}{
            $j \gets \arg\max_{j \in \{j \mid (h,j) \in L_n\}} f(x_{h,j})$ \;
            \If{$f(x_{h,j}) > \nu_{\text{max}}$}{
                ...
                \For{$i \gets 1$ \KwTo $K$}{
                    ...
                    $\mathcal{T}_n \gets \{(x_{h+1,j+i}, f_{h+1,j+i}, \Omega_{n+1})\}$ \;
                }
